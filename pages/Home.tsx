      ...
      <section className="py-24 bg-[#02050c]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { title: 'GovTech Solutions', icon: <ShieldCheck className="h-8 w-8 text-vedha-red" />, desc: 'Secure, scalable systems for public sector transformation.' },
              ...
              <div key={idx} className="glass-panel p-8 rounded-2xl hover:translate-y-[-10px] transition-transform duration-300 group border-t border-white/5 hover:border-t-vedha-red/50">
                <div className="mb-6 p-4 rounded-xl bg-white/5 w-fit group-hover:bg-white/10 group-hover:shadow-[0_0_20px_rgba(239,68,68,0.2)] transition-all">
                  {item.icon}
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 opacity-60 hover:opacity-100 transition-opacity duration-500">
            ...
            {['Secure Infrastructure', 'Cloud Native', 'AI Driven', 'Future Ready'].map((name, i) => (
              <div key={i} className="text-xl md:text-2xl font-display font-bold text-white hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-vedha-blue hover:to-vedha-red transition-all cursor-default border border-white/10 px-6 py-3 rounded-lg bg-white/5">
                {name}
              </div>
            ))}
          </div>
